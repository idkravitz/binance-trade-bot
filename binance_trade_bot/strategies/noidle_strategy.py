...
class Strategy(AutoTrader):
	...
	def scout(self):
		...
		with self.db.db_session() as session:
			...
			timepassed=datetime.now()-last_trade.datetime
			timepassed=timepassed.total_seconds() / 60/60
			if timepassed<=self.maxidlehours:	self.lastupdate=None
			if timepassed>=self.maxidlehours and self.lastupdate is None:
				self.lastupdate=datetime.now()
				self.logger.info(f"Last trade was before {self.maxidlehours} hours! Going to reinit ratios.")
